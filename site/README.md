...
- シリーズは複数ある(`site/build.py` の `SERIES`。usa=アメリカ50州、japan=臥遊風土記)。`text/<シリーズ>/` の章は `_site/<シリーズ>/` に出る
- `## N. 題` → 節見出し(`#sec-N` アンカー付き。章内目次からリンク)
- `### 注` 以下の `注N: …` の段落 → 注のスタイルで表示、`#note-N` アンカー付き(読み返し用)
- `図N: 一行の説明(パス)` + 直後の説明段落 → figure要素。画像ファイルが `text/<シリーズ>/img/` にあれば埋め込み、なければ「画像は準備中」の枠と説明文のみ
- 章冒頭の著者名・プロフィール・目次 → 専用のスタイル
- トップページ = シリーズごとの章の一覧(`authors/index.md` の割り振り表から生成)+ `README.md` 全文

## 地図機能(ウェブ版限定)

章に `site/geo/<シリーズ>/<slug>.json`(章の地図データ)がある場合だけ、build.py がその章のページに次を付ける。正本のMarkdownには一切手を入れない(正本不可侵)ので、読書会の紙面には現れない。
