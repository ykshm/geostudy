...
- ブラウザ検証はリモート実行環境のため、MAPS.md 6節の代替手順による: ブラウザだけがunpkgと地図タイルにTLSで届かないことをcurl(全て200)との対比で確認した上で、_site のコピーのLeafletをローカル退避版に差し替え、タイルを遮断してPlaywright(Chromium)で確認した。リポジトリのテンプレートは変更していない。
